...
0000000000400efc <phase_2>:
  400efc:	55                   	push   %rbp
  400efd:	53                   	push   %rbx
  400efe:	48 83 ec 28          	sub    $0x28,%rsp # making room in the stack frame for 6 numbers
  400f02:	48 89 e6             	mov    %rsp,%rsi
  400f05:	e8 52 05 00 00       	callq  40145c <read_six_numbers> # six numbers are read (1)
  400f0a:	83 3c 24 01          	cmpl   $0x1,(%rsp)
  400f0e:	74 20                	je     400f30 <phase_2+0x34> # now we go to line 375 (2)
  400f10:	e8 25 05 00 00       	callq  40143a <explode_bomb>
  400f15:	eb 19                	jmp    400f30 <phase_2+0x34>
  400f17:	8b 43 fc             	mov    -0x4(%rbx),%eax # %eax: points to what's in prev arg
  400f1a:	01 c0                	add    %eax,%eax # eax: add previous arg together, then compare
  400f1c:	39 03                	cmp    %eax,(%rbx)
  400f1e:	74 05                	je     400f25 <phase_2+0x29> # go to line 371 (4)
  400f20:	e8 15 05 00 00       	callq  40143a <explode_bomb>
  400f25:	48 83 c3 04          	add    $0x4,%rbx
  400f29:	48 39 eb             	cmp    %rbp,%rbx
  400f2c:	75 e9                	jne    400f17 <phase_2+0x1b> # if not equal, go back up to 366 (5) (some sort of loop here)
  400f2e:	eb 0c                	jmp    400f3c <phase_2+0x40> # if equal, we move to 378 (6) and function returns
  400f30:	48 8d 5c 24 04       	lea    0x4(%rsp),%rbx # 0x2 in rbx (my second argument)
  400f35:	48 8d 6c 24 18       	lea    0x18(%rsp),%rbp # 0x19 in rbp (getting 4199473?)
  400f3a:	eb db                	jmp    400f17 <phase_2+0x1b> # go back to line 366 (3)
...
0000000000400f43 <phase_3>:
  400f43:	48 83 ec 18          	sub    $0x18,%rsp # allocate room on stack
  400f47:	48 8d 4c 24 0c       	lea    0xc(%rsp),%rcx # rcx has zero?
  400f4c:	48 8d 54 24 08       	lea    0x8(%rsp),%rdx # looks like this guy has the second arg
  400f51:	be cf 25 40 00       	mov    $0x4025cf,%esi
  400f56:	b8 00 00 00 00       	mov    $0x0,%eax # eax is 2?
  400f5b:	e8 90 fc ff ff       	callq  400bf0 <__isoc99_sscanf@plt>
  400f60:	83 f8 01             	cmp    $0x1,%eax # if greater than 1, continue
  400f63:	7f 05                	jg     400f6a <phase_3+0x27>
  400f65:	e8 d0 04 00 00       	callq  40143a <explode_bomb>
  400f6a:	83 7c 24 08 07       	cmpl   $0x7,0x8(%rsp) # if less than 7, continue
  400f6f:	77 3c                	ja     400fad <phase_3+0x6a> # this will explode bomb
  400f71:	8b 44 24 08          	mov    0x8(%rsp),%eax # OVERALL PATTERN HERE: move, go to 400fbe
  400f75:	ff 24 c5 70 24 40 00 	jmpq   *0x402470(,%rax,8) # eax has zero at this point
  ...
  400fbe:	3b 44 24 0c          	cmp    0xc(%rsp),%eax # eax is loaded with whatever the mov isntruction puts into it, rsp has 6
...
0000000000400fce <func4>:
  400fce:	48 83 ec 08          	sub    $0x8,%rsp # moving the rsp address back by 8
  400fd2:	89 d0                	mov    %edx,%eax # first time around: 14 in edx, gets moved to eax
  400fd4:	29 f0                	sub    %esi,%eax # first time around: 0 in esi, sub 0 from eax
  400fd6:	89 c1                	mov    %eax,%ecx # 14
  400fd8:	c1 e9 1f             	shr    $0x1f,%ecx shift 14 by 31 bits (so ecx is effectively 0)
  400fdb:	01 c8                	add    %ecx,%eax # eax is still 14... seems like we want arg2 to be negative
  400fdd:	d1 f8                	sar    %eax # eax is 7 (left shift by 1)
  400fdf:	8d 0c 30             	lea    (%rax,%rsi,1),%ecx # eax is still 7
  400fe2:	39 f9                	cmp    %edi,%ecx # edi: first arg, ecx: 7 the first time around
  400fe4:	7e 0c                	jle    400ff2 <func4+0x24>
  400fe6:	8d 51 ff             	lea    -0x1(%rcx),%edx
  400fe9:	e8 e0 ff ff ff       	callq  400fce <func4>
  400fee:	01 c0                	add    %eax,%eax
  400ff0:	eb 15                	jmp    401007 <func4+0x39>
  400ff2:	b8 00 00 00 00       	mov    $0x0,%eax # move 0 to eax (used to be 7)
  400ff7:	39 f9                	cmp    %edi,%ecx # 7 and 7 in both registers
  400ff9:	7d 0c                	jge    401007 <func4+0x39> # jump if greater than or equal (so jump happens since 7 = 7) -> (14 in edi > 7 in ecx, jump dont happen)... also we can insinuate from this code that we are looking for a number <=14. Now we can brute force by just testing combinations of (i,0) where <=0 i <= 14
  400ffb:	8d 71 01             	lea    0x1(%rcx),%esi # 8 in esi, 7 in rcx () (looks like esi just gets incremented by 1)
...
000000000040100c <phase_4>:
  40100c:	48 83 ec 18          	sub    $0x18,%rsp # rsp address: 0x7ffe696dd650
  401010:	48 8d 4c 24 0c       	lea    0xc(%rsp),%rcx # rcx: 0x7ffe696dd65c (rsp + 12)... no values in here just yet
  401015:	48 8d 54 24 08       	lea    0x8(%rsp),%rdx # rdx: 0x7ffe696dd658 (rsp + 8)... no values in here just yet
  40101a:	be cf 25 40 00       	mov    $0x4025cf,%esi # this is the string we're reading... "%d, %d"... so definitely two numbers
  40101f:	b8 00 00 00 00       	mov    $0x0,%eax
  401024:	e8 c7 fb ff ff       	callq  400bf0 <__isoc99_sscanf@plt> # returns the number of items successfully matched
  401029:	83 f8 02             	cmp    $0x2,%eax # eax must be 2 so that we don't trigger bomb. arg1 in rsp + 8. arg2 in rsp + 12
  40102c:	75 07                	jne    401035 <phase_4+0x29> # don't want to jump
  40102e:	83 7c 24 08 0e       	cmpl   $0xe,0x8(%rsp) # want to go here... "jump if below or equal 14" ... 0 in rsp + 8 cmpl only compares UNSIGNED, so first arg MUST be greater than zero
  401033:	76 05                	jbe    40103a <phase_4+0x2e>
  401035:	e8 00 04 00 00       	callq  40143a <explode_bomb>
  40103a:	ba 0e 00 00 00       	mov    $0xe,%edx # 14 in edx 
  40103f:	be 00 00 00 00       	mov    $0x0,%esi # 0 in esi
  401044:	8b 7c 24 08          	mov    0x8(%rsp),%edi # 7 in edi (arg1 in edi)
  401048:	e8 81 ff ff ff       	callq  400fce <func4> 
  40104d:	85 c0                	test   %eax,%eax # sets the zero flag when result of AND operation is zero (eax MUST be zero)
  40104f:	75 07                	jne    401058 <phase_4+0x4c> # tests the zero flag and jumps if not set
  401051:	83 7c 24 0c 00       	cmpl   $0x0,0xc(%rsp) # 0 passed in as first arg
...
0000000000401062 <phase_5>:
  ...
  40106a:	64 48 8b 04 25 28 00 	mov    %fs:0x28,%rax # 0x9f0dcb4689043a00 <- value changes every time
  401071:	00 00 
  401073:	48 89 44 24 18       	mov    %rax,0x18(%rsp)
  401078:	31 c0                	xor    %eax,%eax
  40107a:	e8 9c 02 00 00       	callq  40131b <string_length>
  40107f:	83 f8 06             	cmp    $0x6,%eax
  401082:	74 4e                	je     4010d2 <phase_5+0x70> # jump to line 503
  401084:	e8 b1 03 00 00       	callq  40143a <explode_bomb>
  401089:	eb 47                	jmp    4010d2 <phase_5+0x70>
  40108b:	0f b6 0c 03          	movzbl (%rbx,%rax,1),%ecx #continue here (start of loop) -> move first letter into ecx
  40108f:	88 0c 24             	mov    %cl,(%rsp) # -> first letter was placed into rsp (f: 01100110)
  401092:	48 8b 14 24          	mov    (%rsp),%rdx # -> then placed into rdx
  401096:	83 e2 0f             	and    $0xf,%edx #first 4 bits are kept, everything else stays (0110)
  401099:	0f b6 92 b0 24 40 00 	movzbl 0x4024b0(%rdx),%edx # 6 (+ 0x4024b0 -> # is 01110010 -> we have maduiersnfotvbyl here so effectively we have to use our input string as a way to index the string correctly. ex: f's last 4 bits: 0110 -> 6 -> 6th letter is r
  4010a0:	88 54 04 10          	mov    %dl,0x10(%rsp,%rax,1) # not sure what this does stores in dl + 10
  4010a4:	48 83 c0 01          	add    $0x1,%rax
  4010a8:	48 83 f8 06          	cmp    $0x6,%rax # this loop happens six times
  4010ac:	75 dd                	jne    40108b <phase_5+0x29> # loop back to 485 until 0x6 in %rax
  4010ae:	c6 44 24 16 00       	movb   $0x0,0x16(%rsp)
  4010b3:	be 5e 24 40 00       	mov    $0x40245e,%esi
  4010b8:	48 8d 7c 24 10       	lea    0x10(%rsp),%rdi # where our string arg is stored
  4010bd:	e8 76 02 00 00       	callq  401338 <strings_not_equal> # returns 0 if strings not equal?
  4010c2:	85 c0                	test   %eax,%eax 
  4010c4:	74 13                	je     4010d9 <phase_5+0x77> # jumps if eax is 0 to 505
  4010c6:	e8 6f 03 00 00       	callq  40143a <explode_bomb>
  4010cb:	0f 1f 44 00 00       	nopl   0x0(%rax,%rax,1)
  4010d0:	eb 07                	jmp    4010d9 <phase_5+0x77>
  4010d2:	b8 00 00 00 00       	mov    $0x0,%eax
  4010d7:	eb b2                	jmp    40108b <phase_5+0x29> # begin loop...
...
0000000000401338 <strings_not_equal>:
  ...
  40133a:	55                   	push   %rbp 
  40133b:	53                   	push   %rbx 
  40133c:	48 89 fb             	mov    %rdi,%rbx # flyers -> rvfedu... rvfedu -> drreie
  40133f:	48 89 f5             	mov    %rsi,%rbp # flyers (flyers passed in)
